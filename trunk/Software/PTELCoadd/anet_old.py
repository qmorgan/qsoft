...
def anet(imlist,tweak=False):
	a = AstrometrySolver()
	a.get_wcs(imlist=imlist,tweak=tweak)
	print a
...
class AstrometrySolver:
	...
	# default job dict for small fields (0.85arcsec/pix to 2.2)
	job_dict = {"uname": "Adam Morgan","email": "amorgan@astro.berkeley.edu", "fsunit" :"arcsecperpix",\
		"fstype-ul": 1, "fsu": 2.2, "fsl": 0.85, "xysrc": "img", "parity": 2, "index": "10arcmin", "tweak": 0,\
	...
	def _make_request(self,imgfile=None,pixel_size_range = [0.1,2.5], tweak_astrometry=False):
	...
	def get_wcs(self,imlist=None,howmany_at_a_time=5,tweak=False):
			...
			for i in range(nsets):
				...
				for im in imlist[(i*howmany_at_a_time):((i+1)*howmany_at_a_time)]:
					if im.find(".fits") == -1:
						...
						self._make_request(im,pixel_size_range=[0.1,500],tweak_astrometry=tweak)
					else:
						self._make_request(im,tweak_astrometry=tweak)
