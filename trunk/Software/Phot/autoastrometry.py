...
def sextract(sexfilename, nxpix, nypix, border=3, corner=12, minfwhm=1.5, maxfwhm=25, maxellip=0.5):
    ...
    goodsexlist = []
    for sex in sexlist:
       #print sex.x, sex.y, sex.ra, sex.dec, sex.mag, sex.ellip, sex.fwhm,
...
def autoastrometry(filename,pixelscale=-1,pa=-999,inv=0,uncpa=-1, minfwhm=1.5,maxfwhm=20,boxsize=-1,maxrad=-1,tolerance=0.010,catalog=''):
    ...
    if pixelscale > 0 and pa > -360:
       ...
       try: #clobber doesn't work so remove temp.fits manually
           os.remove('temp.fits')
       except:
           pass
    ...
    # Wipe fits info from the header - this will confuse verification
    ...
    rot = offpa * math.pi/180
      #...the image itself
    h.update("CD1_1", cos(rot)*cd11 - sin(rot)*cd12 )
    ...
    #for i in range(len(imraoffset)):
        #print primarymatchs[i], ':', (imraoffset[i]-median(imraoffset))*3600*cos(dec*math.pi/180), (imdecoffset[i]-median(imdecoffset))*3600
    ...
    print '  dra = %.2f",  ddec = %.2f"  (unc. %.3f")' % (raoffset*3600*cos(cdec*math.pi/180), decoffset*3600,stdoffset*3600)
...
def main():
    ...
    writeparfile()
    if not os.path.exists('sex.config'): writeconfigfile()
