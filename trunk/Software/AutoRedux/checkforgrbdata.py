...
cronpath = os.path.expanduser("~/ptelautoredux/")
if not os.path.exists(cronpath):  #create storage directory if it doesn't exist already
    try:
	os.system("mkdir " + cronpath)
	os.system("mkdir " + cronpath + "tmp")
	print "Creating Temporary directory %s and moving files there." % (cronpath)
    except:
	print "CANNOT MAKE TEMP DIRECTORY.  Will try to continue anyway, but will likely fail."
	pass
else: print "Copying temporary files to %s" % (cronpath)

pklflagpath = cronpath+'ptelgrbcheck.pkl'
reduxdir = cronpath+'/tmp/'
...
if os.path.exists(searchpath):
    ...
    if (numfiles > 2) and (pklflagdict['dateflag'] != dateint): 
